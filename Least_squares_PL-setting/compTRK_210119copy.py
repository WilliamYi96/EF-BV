...
# Comparison of EF22 and EF21 under Top-1.
...
####################################################################################
####################################################################################
def ef21_estimator(A, x, b, la, k, g_ar, h_ar, n_workers, compressor='TopK', ratio=1):
...
def ef21(x_0, A, b, A_0, b_0, stepsize, eps, la, k, n_workers, theta, compressor, Nsteps=100000, ratio=1):
    ...
    while stopping_criterion(sq_norm_ar[-1], eps, it, Nsteps):
        ...
        g_ar, h_ar, size_value_sent, grad = ef21_estimator(A, x, b, la, k, g_ar, h_ar, n_workers,
...
####################################################################################
####################################################################################
def ef22_estimator(A, x, b, la, k, g_ar, h_ar, n_workers, compressor='CompK', lamd_ar=0, nu_ar=0, ratio=1):
...
def ef22(x_0, A, b, A_0, b_0, stepsize, eps, la, k, n_workers, theta, compressor, Nsteps=100000, lamd_ar=0, nu_ar=0, ratio=1):
    # Note there is no concept of worker and server in EF21 implementation, as data is sorted at different worker
    ...
    while stopping_criterion(sq_norm_ar[-1], eps, it, Nsteps):
        ...
        g_ar, h_ar, size_value_sent, grad = ef22_estimator(A, x, b, la, k, g_ar, h_ar, n_workers,
                                                           compressor=compressor, lamd_ar=lamd_ar, nu_ar=nu_ar, ratio=ratio)
        g = np.mean(g_ar, axis=0)  # g^(k+1) = mean(g_i(k) + m_i(k))
        sq_norm_ar.append(np.linalg.norm(x=grad, ord=2) ** 2)
        p_ar.append((stepsize / theta) * np.linalg.norm(x=g - grad, ord=2) ** 2)
        it += 1
...
def diana_estimator(A, x, b, la, k, g_ar, h_ar, n_workers, compressor='RandK', lamd_ar=0):
    ...
    # print(lamd_ar)
    g_ar_new = h_ar + d_ar  # instead of g_ar here
    h_ar_new = h_ar + lamd_ar * d_ar
    size_value_sent = sys.getsizeof(delta[0, 0])
    return g_ar_new, h_ar_new, size_value_sent, np.mean(grads, axis=0)
...
def diana(x_0, A, b, A_0, b_0, stepsize, eps, la, k, n_workers, theta, compressor, Nsteps=100000, lamd_ar=0, nu_ar=0):
    # print(stepsize, eps, la, k, n_workers, theta, compressor, lamd_ar, nu_ar)
    # (0.0028182621694899613+0j) 1e-07 0 1 20 0 RandK [0.00892857] 0

    # Note there is no concept of worker and server in EF21 implementation, as data is sorted at different worker
    print(f"DIANA_gd_nw-{n_workers}_k-{k}")
    g_ar = compute_full_grads(A, x_0, b, la, n_workers)
    g = np.mean(g_ar, axis=0)
    sq_norm_ar = [np.linalg.norm(x=g, ord=2) ** 2]
    it_ar = [0]
    ...
    p_ar = [0]
    PRINT_EVERY = 1000
    h_ar = g_ar.copy()  # initialize h_0 = 1 / n sum(h_i^0)

    while stopping_criterion(sq_norm_ar[-1], eps, it, Nsteps):
        x = x - stepsize * g
        f_ar.append(np.mean(compute_full_funcs(A, x, b, la, n_workers)))
        g_ar, h_ar, size_value_sent, grad = diana_estimator(A, x, b, la, k, g_ar, h_ar, n_workers,
                                                            compressor=compressor, lamd_ar=lamd_ar)
        g = np.mean(g_ar, axis=0)  # g^(k+1) = mean(g_i(k) + m_i(k))
...
parser.add_argument('--ratio', type=int, default=1, help='k value instead of the k prime value')
...
for i in range(len(n_ar)):
    ...
    if args.method == 'EF22':
        ...
        # eta_1, omega_1 = np.sqrt(1 - al_ar), 0
        eta_1, omega_1 = eta, omega
        lamd_ar_1 = np.minimum(1, (1 - eta_1) / ((1 - eta_1) ** 2 + omega_1))
        nu_ar_1 = np.minimum(1, (1 - eta_1) / ((1 - eta_1) ** 2 + omega_1))
        r_1 = (1 - lamd_ar_1 + lamd_ar_1 * eta_1)**2 + lamd_ar_1**2 * omega_1
        r_av_1 = (1 - nu_ar_1 + nu_ar_1 * eta_1)**2 + nu_ar_1**2 * omega_1
        ss_1 = np.sqrt((1 + r_1) / (2 * r_1)) - 1
        theta_ar_1 = ss_1 * (1 + ss_1) * r_1 / r_av_1
        gamma_1 = 1. / (L_0 + Lt * np.sqrt(r_av_1 / r_1) * (1. / ss_1)) * factor
        if gamma > gamma_1:
            print(f'EF22 improves EF21 by {gamma - gamma_1}')
        else:
            print(f'EF21 has better learning rate than EF22 by {gamma_1-gamma}')
        # assert 0
        print(lamd_ar, nu_ar, r, r_av, ss, theta_ar, gamma)
        print(lamd_ar_1, nu_ar_1, r_1, r_av_1, ss_1, theta_ar_1, gamma_1)
        # assert 0
    ...
    experiment_name = "{3}-full-grad_nw-{0}_{1}x_{2}_{4}".format(n_ar[i], factor, compressor, args.method, args.ratio)
    ...
    for k in range(len(k_ar)):
        if args.method == 'DIANA':
            ...
            results = ef22(x_0, X, y, X_0, y_0, gamma[k], eps, la, k_ar[k], n_ar[i],
                           theta_ar[k], compressor=compressor, Nsteps=nsteps, lamd_ar=lamd_ar, nu_ar=nu_ar, ratio=ratio)
