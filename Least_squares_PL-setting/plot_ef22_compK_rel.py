...
def plot1(x_ar, y_ar, label_ar, plot_path, dataset, title=None, xaxis='bits/n', yaxis=r"$\|| \nabla f(x) \||^2$",
    ...
    # print (len(x_ar), len(marker_ar), len(color_ar_1))  # (10, 27, 21)
...
for k_size_ar_bd in k_size_ar_bd_ar:
    ...
    # main_title = F"EF22_{dataset}_20workers_300K_CompK"
    # main_title = f'EF21_Top_1_{dataset}_20workers_300K'
    # main_title = f'EF22_CompK1_{k_size_ar_bd}_{dataset}_20workers_300K'
    # main_title = f'EF22_CompK1_all_{dataset}_20workers_300K'
    # main_title = f'EF21_CompK1_{k_size_ar_bd}_{dataset}_20workers_300K'
    main_title = f'EF22_CompK_{k_size_ar_bd}_{dataset}_20workers_3000K'

    # factor_ar = [4,8]
    # factor_ar_bd = [1,2,4,8,16,32,64,128,256,512,1024,2048]
    # factor_ar_bd = [1, 4, 8, 16, 32]
    # factor_ar_bd = [8]
    # factor_ar_bd = [1, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192]
    ...
    # experiment_ar = ["EF22-full-grad_nw-{0}_{1}x_CompK_{2}".format(n, factor_bd, k) for n, k, factor_bd in
    #                  itertools.product(n_ar, k_size_ar_bd, factor_ar_bd)]
    # experiment_ar = ["EF21-full-grad_nw-{0}_{1}x_TopK_{2}_1".format(n, factor_bd, k) for n, k, factor_bd in
    #                  itertools.product(n_ar, k_size_ar_bd, factor_ar_bd)]  # #1
    experiment_ar = ["EF22-full-grad_nw-{0}_{1}x_CompK_{2}_True_1".format(n, factor_bd, k) for n, k, factor_bd in
                     itertools.product(n_ar, k_size_ar_bd, factor_ar_bd)]  # #1
    ...
    for i, experiment in enumerate(experiment_ar):
        ...
        # sq_norm = np.load(logs_path + 'diff_ar' + '_' + experiment + '.npy')
        # sq_norm_ar.append(sq_norm[:number_its])
        f_ar = np.load

        print(experiment, f"iter: {its_ar[-1].shape[0]};", f"bits/n: {its_ar[-1][-1]}")

    filename1 = "{0}_{1}_{2}.pdf".format(main_title, dataset, ratio)
    filename2 = "{0}_{1}_{2}_sq.pdf".format(main_title, dataset, ratio)
    # filename = "{0}.pdf".format(experiment_ar)
    use_top_iter = 4
    diff_ar = []
    for i in range(len(sq_norm_ar)):
        diff_ar.append(np.mean(sq_norm_ar[i], axis=0))
    print(diff_ar[0].shape, len(diff_ar))
    print(sq_norm_ar[0])
    # plot1(its_ar, norms_ar, label_ar, plot_path, dataset, filename=filename1, save=1)
    plot1(its_ar, diff_ar, label_ar, plot_path, dataset,
          filename=filename2, save=1, yaxis=r"$\|| f(x) - f(x^\star) \||^2$")
