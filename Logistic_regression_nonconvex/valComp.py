"""
experiment for logistic regression function with non-convex regularizer EF22
"""
import numpy as np
...
parser.add_argument('--num_workers', action='store', dest='num_workers', type=int, default=1,
...
num_workers = [20, 50, 100, 1000]
ratios = [1, 2, 5, 10, 56, 112]

file_open = f'rel/{args.dataset}/{args.method}_{args.compressor}_rel.txt'

for num_worker in num_workers:
    n_ar = np.array([num_worker])
    k_ar = np.array([k_size])
    factor = args.factor
    eps = args.tol
    la = args.reg_rate

    if args.overlap > num_worker:
        continue
    ...
    if args.overlap == 1:
        data_path = project_path + "data_{0}_{1}/".format(dataset, num_worker)
    else:
        data_path = project_path + "splits/data_{0}_{1}_{2}/".format(dataset, num_worker, args.overlap)
    ...
    hess_f_0 = (1 / (4 * n_0)) * (X_0.T @ X_0) + la  # TODO: how this hessian can be computed?
    L_0 = np.max(np.linalg.eigvals(hess_f_0))
    L_0 = L_0.astype(np.float)
    mu_0 = np.min(np.abs(np.linalg.eigvals(hess_f_0)))
    mu_0 = mu_0.astype(np.float)  # mu_0 is not being used
    ...
    for i in range(len(n_ar)):
        ...
        for ratio in ratios:
            ...
            if args.method == 'EF22':
                ...
                if compressor == 'MixK': # MixK is a contrastive compressor
                    lamd_ar = nu_ar = np.array([1])
                ...
                gamma = 1 / (1 + 6 * omega / num_worker) / L_0 * factor
                theta_ar = [0]
                lamd_ar = [1 / (omega + 1)]
                nu_ar = [1]
            ...
            with open(file_open, 'a+') as f:
                if args.method == 'DIANA':
                    line = f'{num_worker}, {args.overlap}, {ratio}, {lamd_ar[0]}, {nu_ar[0]}, {gamma[0]}\n'
                    print(num_worker, ratio, lamd_ar[0], nu_ar[0], gamma[0])
                else:
                    line = f'{num_worker}, {args.overlap}, {ratio}, {eta[0]}, {omega[0]}, {omega_av[0]}, ' \
                           f'{lamd_ar[0]}, {nu_ar[0]}, {r[0]}, {r_av[0]}, {ss[0]}, {gamma[0]}\n'
