...
def biased_diana_rand_k_gd(x_0, A, b, A_0, b_0, stepsize, eps, la, k, n_workers, ind, Nsteps=100000):
    ...
    while stopping_criterion(sq_norm_ar[-1], eps, it, Nsteps):
        x = x - stepsize * g
        g_ar, size_value_sent, grad = biased_diana_rand_k_gd_estimator(A, x, b, la, k, g_ar, n_workers, ind)
        g = np.mean(g_ar, axis=0)
...
ind = False   # dependent sampling K or independent sampling
...
for i in range(len(n_ar)):
    ...
    experiment_name = "biased-diana-full-grad_nw-{0}_{1}x_randK_ind_{2}".format(n_ar[i], factor, ind)
    its_bdfg_tpc = []
    norms_bdfg_tpc = []
    sol_bdfg_tpc = []
    ...
    for k in range(len(k_ar)):
        results = biased_diana_rand_k_gd(x_0, X, y, X_0, y_0, step_size_diana_tpc[k], eps, la, k_ar[k], n_ar[i],
                                        ind=ind, Nsteps=nsteps)
