"""
Logistic regression with convex regularizer.
...
parser.add_argument('--reg_rate', type=float, default=0.001, help='regularizer rate: 0.1|0.01, default 0.1')
...
hess_f_0 = (1 / (4 * n_0)) * (X_0.T @ X_0) + la * np.eye(d_0)  # This is with the convex regularizer
...
for i in range(len(n_ar)):
    ...
    for j in range(n_ar[i]):
        ...
        hess_f_j = (1 / (4 * n[j])) * (X[j].T @ X[j]) + la * np.eye(d[j])
    ...
    experiment_name = f"{args.method}-full-grad_nw-{n_ar[i]}_{factor}x_{compressor}_{args.ratio}_{args.ind}_{args.overlap}_convex"
    its_bdfg_tpc = []
    norms_bdfg_tpc = []
    sol_bdfg_tpc = []
    f_ar = []
