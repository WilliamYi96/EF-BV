...
k_prime_size_ar = [34]  # choose from 10 and 34
k_size_ar = [1]  # Perm-(k', k), k value arrays
lr_factor_ar = [1]  # learning rate multiply factor, >1 for stepsize tolerance checking
overlap_ar = [1]  # overlap ratio, choose from 1 and 2

for k_size_ar_bd in k_size_ar:

    n_ar = np.array([1000], dtype=int)

    # We traverse k_size_ar, but select the only value k', lr_factor, overlap_factor
    main_title = f'CompK_{k_prime_size_ar[0]}_{k_size_ar_bd}_{dataset}_11055workers_3000K_{n_ar[0]}'
    ...
    filename = f"{main_title}_{dataset}_{k_prime_size_ar[0]}_{k_size_ar[0]}_{lr_factor_ar[0]}_{overlap_ar[0]}_logreg_diff.pdf"
    use_top_iter = 4
    # interval = 50000  # a9a
    # interval = 80000  # w8a
    interval = 60000 # phishing, osilation
    # interval = int(len(its_ar[0]) / 2) # mushrooms
    # interval = 4500  # phishing
