...
parser = argparse.ArgumentParser(
    description='Generate data and provide information about it for workers and parameter server')
...
parser.add_argument('--loss_func', action='store', dest='loss_func', type=str, default="log_reg",
                    help='loss function ')
parser.add_argument('--num_starts', action='store', dest='num_starts', type=int, default=1,
                    help='Number of starts from different points')
parser.add_argument('--shared_start', action='store', dest='shared_start', type=int, default=1,
                    help='Whether or not each launch experiments from differnt point')  # if shared_start == 1 then each launch starts from the same point
...
data_path = project_path + "data/data_{0}_{1}/".format(dataset, num_workers)
...
for i in range(len(X)):
    print(
        f"worker {i} has {X[i].shape[0]} datasamples; class 1: {X[i][np.where(y[i] == 1)].shape[0]}; class -1: {X[i][np.where(y[i] == -1)].shape[0]}")
