...
def main():
    ...
    parser.add_argument('--max_it', action='store', dest='max_it', type=int, default=3000,
    ...
    parser.add_argument('--num_workers', action='store', dest='num_workers', type=int, default=5,
    ...
    parser.add_argument('--batch_size', action='store', dest='batch_size', type=int, default=1024,
                        help='Batch size per worker and for GPU')
    parser.add_argument('--model', action='store', dest='model', type=str, default='resnet18',
                        help='Name of NN architechture')
    parser.add_argument('--dataset', action='store', dest='dataset', type=str, default='CIFAR10',
                        help='Name of dataset')
    parser.add_argument('--K_prime', type=int, default=10, help="K' value")
    ...
    # =======================================================================================================
    # Load data
    train_sets, train_set_full, test_set, train_loaders, test_loaders, classes = utils.getSplitDatasets(
        nn_config.dataset, nn_config.batch_size,
        nn_config.load_workers, kWorkers)

    print(f"Start training {nn_config.model_name}@{nn_config.dataset} for K={nn_config.KMax} iteration. EF21",
          available_devices)
    ...
    # nn_config.i_use_vr_biased_diana = True  # EF21
    nn_config.i_use_vr_biased_diana = False
    nn_config.i_use_efbv = True
    ...
    if nn_config.i_use_vr_marina:
        ...
        nn_config.gamma = gamma  # Gamma for VR-BIASED-DIANA, learning rate
        nn_config.batch_size_for_worker = batch_size_for_worker

    elif nn_config.i_use_efbv:
        # Set necessary parameters
        # TODO: implement c_top and makeTopKCompressor
        c_permK = compressors.Compressor()
        c_permK.makeCompKCompressor(int(K_prime), int(K), nn_config.D)
        nn_config.gamma = gamma  # For DNN experiments, gamma is deliberatlly fixed.
        nn_config.batch_size_for_worker = batch_size_for_worker
    ...
    for i in range(kWorkers):
        ...
        worker_cfgs[-1].compressor.makeTopKCompressor(int(K), nn_config.D)  # TODO: check whether this should be modified
        ...
        for iteration in range(0, nn_config.KMax):
            ...
            if iteration % 10 == 0:
                ...
                test_acc[iteration] = getAccuracy(worker_cfgs[0].model, test_set, nn_config.batch_size,
                                                  worker_cfgs[0].device)
                print(
                    f"  train accuracy: {train_acc[iteration]}, test accuracy: {test_acc[iteration]}, "
                    f"train loss: {train_loss[iteration]}, test loss: {test_loss[iteration]}")
                print(
