...
class CompressorType:
    ...
    RANDK_COMPRESSOR = 3  # Rand-K compressor
    ...
    COMPK_COMPRESSOR = 11  # CompK compressor, newly added
    MIXK_COMPRESSOR = 12 # Mix-K compressor, newly added. Currently not support
...
class Compressor:
    ...
    def name(self):
        ...
        if self.compressorType == CompressorType.COMPK_COMPRESSOR: return f"Comp-(K'={self.K_prime}, K={self.K})"   # TODO: add self.K_prime
        return "?"
    ...
    def fullName(self):
        ...
        if self.compressorType == CompressorType.COMPK_COMPRESSOR: return  f"Comp-(K'={self.K_prime}, K={self.K}), D={self.D}"
        return "?"
    ...
    def makeStandardDitheringFP64(self, levels, vectorNormCompressor, p=float("inf")):
        ...
        self.levelsValues = np.arange(0.0, 1.1,
                                      1.0 / levels)  # levels + 1 values in range [0.0, 1.0] which uniformly split this segment
    ...
    def makeStandardDitheringFP32(self, levels, vectorNormCompressor, p=float("inf")):
        ...
        self.levelsValues = torch.arange(0.0, 1.1,
                                         1.0 / levels)  # levels + 1 values in range [0.0, 1.0] which uniformly split this segment
        self.s = len(self.levelsValues) - 1  # should be equal to level
        assert self.s == levels
    ...
    def makeNaturalDitheringFP64(self, levels, dInput, p=float("inf")):
        ...
        r = min(p, 2)
        self.w = 1.0 / 8.0 + (dInput ** (1.0 / r)) / (2 ** (self.s - 1)) * min(1, (dInput ** (1.0 / r)) / (
                    2 ** (self.s - 1)))
        self.resetStats()
    ...
    def makeCompKCompressor(self, K_prime, K, D):
        self.compressorType = CompressorType.COMPK_COMPRESSOR
        self.D = D
    ...
    def compressVector(self, x):
        ...
        if self.compressorType == CompressorType.IDENTICAL:
        ...
        elif self.compressorType == CompressorType.COMPK_COMPRESSOR:
            # TODO: add neccesary code here
            x = 0
            ...
            if self.compressorType == CompressorType.NATURAL_DITHERING_FP64:
                self.last_need_to_send_advance = d * (
                            1.0 + math.ceil(math.log2(self.s))) / 64.0  # 1 bit for sign log2(levels) bits to send level
            elif self.compressorType == CompressorType.NATURAL_DITHERING_FP32:
                self.last_need_to_send_advance = d * (
                            1.0 + math.ceil(math.log2(self.s))) / 32.0  # 1 bit for sign log2(levels) bits to send level
