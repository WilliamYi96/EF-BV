...
def rand_k_matrix(X, k, ind=False):
    ...
    dim = X.shape[1]
    idxs = None if ind else np.random.choice(dim, k, replace=False)  # Fix this bug, replace should be set as False
...
def rand_k_compressor(x, k, ind, idxs=None):
    ...
    if ind: idxs = np.random.choice(dim, k, replace=False)  # The same, replace should be set as False
...
def mix_k_compressor(x, k, ind=True, inds_rand=None):
    ...
    if ind:
        inds_rand = np.random.choice(rand_idx, k, replace=False)
...
def comp_k_compressor(x, kl, ks, ind=True, inds_rand=None):
    ...
    if ind:
        inds_rand = np.random.choice(inds_top, ks, replace=False)
